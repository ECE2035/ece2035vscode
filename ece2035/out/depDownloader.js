...
function checkDependencies(context) {
    (0, vscode_file_downloader_api_1.getApi)().then((api) => {
        // getting latest version string
        api.downloadFile(vscode.Uri.parse(baseUrl + "version.txt"), "riscv-version.txt", context).then((fileUri) => {
            // read the file
            let fs = require("fs");
            ...
            vscode.window.showInformationMessage("A new version of the RISC-V Emulator is available. Would you like to update?", "Yes", "No").then((value) => {
                if (value === "Yes") {
                    ...
                    api.downloadFile(vscode.Uri.parse(url), "riscvemulator.exe", context).then((fileUri) => {
...
//# sourceMappingURL=depDownloader.js.map